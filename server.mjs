...
CREATE TABLE IF NOT EXISTS transfers(id TEXT PRIMARY KEY,sender_id TEXT NOT NULL,title TEXT NOT NULL,status TEXT NOT NULL DEFAULT 'uploading',created_at INTEGER NOT NULL,total_bytes INTEGER NOT NULL DEFAULT 0,message_text TEXT,FOREIGN KEY(sender_id) REFERENCES users(id));
...
try { db.exec('ALTER TABLE transfers ADD COLUMN message_text TEXT'); } catch (e) { if (!String(e).includes('duplicate column')) throw e; }
...
const server=http.createServer(async(req,res)=>{
  ...
  try {
    ...
    if(req.method==='GET'&&url.pathname==='/api/transfers'){
      const u=requireUser(req,res);if(!u)return; const rows=db.prepare(`SELECT t.id,t.title,t.message_text,t.status,t.created_at,t.total_bytes,s.name sender_name,t.sender_id,CASE WHEN EXISTS(SELECT 1 FROM recipients mx WHERE mx.transfer_id=t.id AND mx.user_id=?) THEN (SELECT mr.seen_at FROM recipients mr WHERE mr.transfer_id=t.id AND mr.user_id=?) ELSE -1 END seen_at,EXISTS(SELECT 1 FROM recipients mr WHERE mr.transfer_id=t.id AND mr.user_id=?) is_recipient,COUNT(DISTINCT f.id) file_count,GROUP_CONCAT(DISTINCT ru.name) recipients FROM transfers t JOIN users s ON s.id=t.sender_id LEFT JOIN transfer_files f ON f.transfer_id=t.id LEFT JOIN recipients r ON r.transfer_id=t.id LEFT JOIN users ru ON ru.id=r.user_id WHERE t.sender_id=? OR r.user_id=? OR ?='admin' GROUP BY t.id ORDER BY t.created_at DESC`).all(u.id,u.id,u.id,u.id,u.id,u.role);return json(res,200,rows);
    }
    ...
    if(req.method==='POST'&&url.pathname==='/api/uploads'){
      const u=requireUser(req,res);if(!u)return;const b=await readJson(req);const text=String(b.text||'').trim().slice(0,10000);const rawFiles=Array.isArray(b.files)?b.files:[];if(!rawFiles.length&&!text)return json(res,400,{error:'متن بنویسید یا فایلی انتخاب کنید'});if(!Array.isArray(b.recipients)||!b.recipients.length)return json(res,400,{error:'گیرنده‌ای انتخاب نشده است'});
      const files=rawFiles.map(x=>({path:safeRelativePath(x.path),size:Number(x.size)}));if(files.some(x=>!Number.isSafeInteger(x.size)||x.size<0||x.size>config.maxFileBytes))return json(res,400,{error:'اندازه یکی از فایل‌ها مجاز نیست'});const total=files.reduce((a,x)=>a+x.size,0);if(storageUsed()+total>config.storageLimitBytes-config.reserveBytes)return json(res,507,{error:'فضای میزبان برای این ارسال کافی نیست'});
      ...
      const id=randomId(),status=files.length?'uploading':'ready',title=text?text.slice(0,60):files[0].path;db.exec('BEGIN');try{db.prepare('INSERT INTO transfers(id,sender_id,title,status,created_at,total_bytes,message_text) VALUES(?,?,?,?,?,?,?)').run(id,u.id,title,status,Date.now(),total,text||null);const insert=db.prepare('INSERT INTO transfer_files VALUES(?,?,?,?,?,0)');const result=[];for(const f of files){const fid=randomId();const stored=randomId(24);insert.run(fid,id,f.path,stored,f.size);result.push({id:fid,path:f.path,size:f.size,uploaded:0});}const ir=db.prepare('INSERT INTO recipients VALUES(?,?,NULL,0)');for(const rid of validRecipients)ir.run(id,rid);db.exec('COMMIT');audit(req,u,files.length?'upload_created':'message_sent','transfer',id,JSON.stringify({total,recipients:validRecipients.length,hasText:!!text}));if(!files.length)broadcast('transfer',{id,title:'پیام جدید',sender:u.name},validRecipients);return json(res,201,{id,files:result,ready:!files.length});}catch(e){db.exec('ROLLBACK');throw e;}
    }
